...
class PdfEditorWindow(Gtk.ApplicationWindow):
            ...
            for block in blocks:
                ...
                for line in block["lines"]:
                    for span in line["spans"]:
                        ...
                        # Store the original color format
                        original_color = span.get("color", (0, 0, 0))
                        ...
                        editable_text = EditableText(
                            ...
                            color=original_color,  # Pass the original color
                            text_instance=span
                        )
                        ...
                        # Store the original color separately
                        editable_text.original_color = original_color
    ...
    def normalize_color(self, color):
        if isinstance(color, (int, float)):
            # Single value represents grayscale
            val = float(color)
            if val > 1:  # If value is > 1, assume it's 0-255 range
                val = val / 255.0
            val = max(0, min(1, val))  # Clamp between 0 and 1
            return (val, val, val)
        elif isinstance(color, (list, tuple)):
            if len(color) >= 3:
                # Convert RGB/RGBA values to 0-1 range if they're in 0-255
                rgb = list(color[:3])  # Take first 3 values for RGB
                for i in range(3):
                    if isinstance(rgb[i], (int, float)):
                        if rgb[i] > 1:
                            rgb[i] = rgb[i] / 255.0
                        rgb[i] = max(0, min(1, rgb[i]))  # Clamp between 0 and 1
                return tuple(rgb)
    ...
    def apply_text_changes(self):
        ...
        try:
            ...
            y_offset = text.baseline
            line_height = text.font_size * 1.2
            
            # Use the original color stored in the text object
            original_color = text.original_color
            normalized_color = self.normalize_color(original_color)
            
            for line in lines:
                if line.strip():
                    font_name = text.font_family
                    insert_point = (text.x, y_offset)
                    page.insert_text(
                        point=insert_point,
                        text=line,
                        fontname=font_name,
                        fontsize=text.font_size,
                        color=normalized_color  # Use the normalized original color
                    )
    ...
    def setup_text_entry(self, text):
        ...
        done_button.connect("clicked", self.on_done_editing, text)
        ...
        entry.get_buffer().set_text(text.text)
        ...
        self.main_box.append(outer_box)
        
        entry.grab_focus()
        
        entry.get_buffer().connect('changed', self.on_text_entry_changed, text)
        
        focus_controller = Gtk.EventControllerFocus.new()
        focus_controller.connect('leave', self.on_text_entry_focus_out, text)
        entry.add_controller(focus_controller)
        
        self.text_entry = outer_box

    def on_done_editing(self, button, text):
        if self.text_entry:
            scroll = self.text_entry.get_last_child()
            text_view = scroll.get_child()
            buffer = text_view.get_buffer()
            start_iter = buffer.get_start_iter()
            end_iter = buffer.get_end_iter()
            final_text = buffer.get_text(start_iter, end_iter, True)
    ...
    def on_text_entry_changed(self, buffer, text):
        start_iter = buffer.get_start_iter()
        end_iter = buffer.get_end_iter()
