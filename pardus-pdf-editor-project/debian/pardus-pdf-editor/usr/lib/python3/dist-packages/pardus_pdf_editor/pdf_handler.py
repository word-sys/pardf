    ...
    try:
        ...
        if not text_obj.is_new and text_obj.bbox:
            ...
            if redact_rect.is_empty or not redact_rect.is_valid:
                ...
                apply_result = page.apply_redactions(images=fitz.PDF_REDACT_IMAGE_NONE, graphics=1) # Use value 1 for removing graphics/text
                if not apply_result:
...
def save_document(doc, save_path):
    """Saves the document using fitz."""
    if not doc:
        return False, "No document to save."
    try:
        # Clean pages before final save can help reduce size and fix issues
        # This can be slow for large documents. Consider making it optional.
        # print("Cleaning document contents before saving...")
        # for page in doc:
        #    page.clean_contents()
        # print("Cleaning done.")

        # Save with garbage collection, compression, and incremental update disabled (creates smaller file)
        doc.save(save_path, garbage=4, deflate=True, incremental=False)
        return True, None
    except Exception as e:
