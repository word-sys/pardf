...
class PdfEditorWindow(Adw.ApplicationWindow):
    ...
    def _build_ui(self):
        ...
        # Placeholder Label
        self.empty_label = Gtk.Label(label="Open or drop a PDF file.", vexpand=True,
    ...
    def _create_sidebar(self):
        ...
        self.select_tool_button = Gtk.Button(icon_name="input-mouse-symbolic", label="Select")
        self.select_tool_button.set_tooltip_text("Select Text (Default)")
        ...
        self.add_text_tool_button = Gtk.Button(icon_name="insert-text-symbolic", label="Add Text")
        self.add_text_tool_button.set_tooltip_text("Add New Text Box")
    ...
    def _create_main_toolbar(self):
        ...
        self.font_combo.set_tooltip_text("Font Family (for new/edited text)")
    ...
    def _finish_loading(self, doc, error_msg, filepath):
        ...
        if error_msg:
            ...
            self.status_label.set_text("Failed to load document.")
            ...
            self.status_label.set_text(f"Loading thumbnails...")
        ...
        def _load_next_thumb():
            if self.thumb_load_iter < page_count:
                 ...
                 if index % 5 == 0 or index == page_count - 1:
                     self.status_label.set_text(f"Loaded thumbnail {index + 1}/{page_count}")
                 return GLib.SOURCE_CONTINUE # Schedule next load
            else:
                 # Finished loading thumbnails
                 self.status_label.set_text(f"Loaded: {os.path.basename(self.current_file_path)}")
    ...
    def save_document(self, save_path):
        ...
        success, error_msg = pdf_handler.save_document(self.doc, save_path)
        ...
        if success:
            ...
            show_error_dialog(self, f"Error saving PDF: {error_msg}")
    ...
    def _apply_and_hide_editor(self, force_apply=False):
        ...
        if force_apply or text_actually_changed or text_obj_to_apply.modified:
            self.status_label.set_text("Applying text changes...")
            ...
            if success:
                ...
                self.status_label.set_text("Text changes applied.")
            else:
                show_error_dialog(self, f"Failed to apply text changes: {error_msg}")
                # Optionally reload page anyway to revert visual state?
                self._load_page(self.current_page_index)
                self.status_label.set_text("Failed to apply text changes.")
        else:
    ...
    def check_unsaved_changes(self):
        ...
        if self.document_modified:
            confirm = show_confirm_dialog(self,
                                          "There are unsaved changes. Do you want to save them before closing/opening?",
            ...
            if confirm: # User wants to save
                 if self.current_file_path:
                     ...
                     show_error_dialog(self, "Please use 'Save As...' first to save the changes.")
    ...
    def on_open_clicked(self, button):
        ...
        dialog = Gtk.FileChooserDialog(title="Open PDF File", transient_for=self, modal=True,
    ...
    def on_save_clicked(self, button):
        if self.current_file_path:
            ...
            self.on_save_as(None, None) # Trigger Save As

    def on_save_as(self, action, param):
        if not self.doc: return

        dialog = Gtk.FileChooserDialog(title="Save PDF As...", transient_for=self, modal=True,
    ...
    def on_export_as(self, action, param):
        ...
        dialog = Gtk.FileChooserDialog(title="Export As...", transient_for=self, modal=True,
                                       action=Gtk.FileChooserAction.SAVE)
    ...
    def _execute_export(self, format_name, output_path):
        ...
        self.status_label.set_text(f"Exporting as {format_name}...")

        success = False
        error_msg = "Unknown export format."
        ...
        try:
            if format_name == "DOCX":
                ...
                success, error_msg = pdf_handler.save_document(self.doc, output_path)
                if success:
                     self.document_modified = False # Saving clears modified flag if successful
                     self._update_ui_state()
            else:
                 success = False # Handled by initial error_msg

            if success:
                 self.status_label.set_text(f"Document exported as {format_name}: {os.path.basename(output_path)}")
            else:
                 show_error_dialog(self, f"Export Failed: {error_msg}")
                 self.status_label.set_text(f"Export as {format_name} failed.")

        except Exception as e:
             show_error_dialog(self, f"Unexpected error during export: {e}")
             self.status_label.set_text("Export failed.")
    ...
    def update_page_label(self):
        ...
        self.page_label.set_text(f"Page {self.current_page_index + 1} of {count}" if count > 0 else "Page 0 of 0")
    ...
    def on_pdf_view_pressed(self, gesture, n_press, x, y):
        ...
        if self.tool_mode == "select":
            ...
            new_text_obj = EditableText(
                x=page_x, y=page_y, text="New Text", # Default text
                font_size=font_size,
    ...
    def on_key_pressed(self, controller, keyval, keycode, state):
        ...
        # Delete Key
             ...
             if self.selected_text and not (self.text_edit_popover and self.text_edit_popover.is_visible()):
                  ...
                  confirm = show_confirm_dialog(self, f"Delete the selected text?\n'{self.selected_text.text[:50]}...'")
                  if confirm:
                       self.status_label.set_text("Deleting text...")
                       ...
                       if success:
                            ...
                            self._load_page(self.current_page_index)
                            self.status_label.set_text("Text deleted.")
                       else:
                            show_error_dialog(self, f"Failed to delete text: {error_msg}")
                            self.status_label.set_text("Failed to delete text.")
    ...
    def on_tool_selected(self, button, tool_name):
        ...
        if self.text_edit_popover and self.text_edit_popover.is_visible():
             ...
             print("Applying changes before switching tool...")
        ...
        print(f"Tool changed to: {self.tool_mode}")
