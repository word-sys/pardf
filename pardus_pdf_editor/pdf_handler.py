...
from .utils import find_specific_font_variant, UNICODE_FONT_PATH
...
def draw_page_to_cairo(cr, doc, page_index, zoom_level): # cr is now page_cr
    """Renders a PDF page onto a Cairo context using GdkPixbuf.
       The provided 'cr' is expected to be the size of the page.
    """
    if not doc or not (0 <= page_index < doc.page_count):
    ...
    try:
        ...
        samples_bytes = bytes(pix.samples) # Ensure it's bytes
        ...
        if pixbuf:
            cr.set_source_rgb(1.0, 1.0, 1.0) # White page background
            cr.paint()

            # Use the global Gdk.cairo_set_source_pixbuf function
            Gdk.cairo_set_source_pixbuf(cr, pixbuf, 0, 0) # CORRECTED LINE
            cr.paint() # Paint the PDF content itself
            return True, None
        else:
...
def extract_editable_text(doc, page_index):
    ...
    try:
        page = doc.load_page(page_index)
        # flags=11 includes font flags needed for bold/italic detection
            ...
            if block.get("type") == 0: # Text block
                for line in block.get("lines", []):
                    for span in line.get("spans", []):
                        ...
                        # Create EditableText, passing the full span_data
                        editable = EditableText(
                            x=bbox[0], y=bbox[1], text=text,
                            font_size=span.get("size", 11),
                            color=span.get("color", 0),
                            span_data=span, # Pass the whole dictionary
...
def apply_text_edit(doc, text_obj: EditableText, new_text: str):
    """Applies changes. Attempts to find specific styled TTF font.
       Falls back to generic UNICODE_FONT_PATH or Base14.
    """
    ...
    # 1. Try to find specific TTF variant based on detected family and style
    if text_obj.font_family_base: # Check if we have a base name to search for
        specific_font_path = find_specific_font_variant(
    ...
    if specific_font_path:
        # Found specific B/I variant - use it for embedding!
        internal_font_name = f"PardusEdit_{text_obj.font_family_base.replace(' ','')}" # Create unique internal name
        font_arg = {"fontfile": specific_font_path, "fontname": internal_font_name}
        print(f"Using specific TTF: {specific_font_path} for Style(B={text_obj.is_bold}, I={text_obj.is_italic})")
    elif UNICODE_FONT_PATH:
        # Didn't find specific variant, but have a generic fallback TTF
        internal_font_name = "PardusEditFont_Generic"
        font_arg = {"fontfile": UNICODE_FONT_PATH, "fontname": internal_font_name}
        print(f"Warning: Could not find specific TTF for '{text_obj.font_family_base}' (B={text_obj.is_bold}, I={text_obj.is_italic}). Using generic fallback: {UNICODE_FONT_PATH}. Style may not apply.")
    else:
        # No TTF found at all, fall back to Base14
        pdf_font_name = _get_base14_font_variant(
            text_obj.pdf_fontname_base14, # Use the Base14 category name
            text_obj.is_bold,
            text_obj.is_italic
        )
        font_arg = {"fontname": pdf_font_name}
        print(f"CRITICAL WARNING: No TTF fonts found. Using Base 14 font: {pdf_font_name}. Unicode will likely fail.")
        # Check for non-ASCII chars
        if any(ord(c) > 127 for c in new_text):
            print("Error: Cannot save non-ASCII text with Base 14 fonts.")
            return False, "Cannot save non-ASCII text: No suitable Unicode font found."

    # --- Start PDF Modification ---
    try:
        page = doc.load_page(text_obj.page_number)
        print(f"DEBUG: Loaded page {text_obj.page_number} for editing.")

        # --- 1. Redact Original Area (Using simple Redaction) ---
        if not text_obj.is_new and text_obj.bbox:
            # ... (Redaction code as restored previously) ...
            print(f"DEBUG: Processing existing text '{text_obj.original_text[:30]}...' using Redaction")
            redact_rect = fitz.Rect(text_obj.bbox); redact_rect.normalize()
            page_rect = page.rect; cover_rect = redact_rect.intersect(page_rect)
            if not cover_rect.is_empty and cover_rect.is_valid:
                print(f"DEBUG: Adding basic redaction annotation for rect: {cover_rect}")
                try:
                    annot = page.add_redact_annot(cover_rect)
                    if annot:
                        print("DEBUG: Applying redactions (basic call)...")
                        apply_result = page.apply_redactions()
                        print(f"DEBUG: apply_redactions result: {apply_result}")
                    else: print("Warning: Could not add redaction annotation.")
                except Exception as e_redact: print(f"ERROR during redaction: {e_redact}")
            else: print(f"Warning: Invalid redaction rectangle derived from bbox: {text_obj.bbox}")

        # --- 2. Insert New Text ---
        if new_text:
            print(f"DEBUG: Inserting new text '{new_text[:30]}...' using font args: {font_arg}")
            # ... (text insertion loop using insert_text and font_arg as before) ...
            insert_point = fitz.Point(text_obj.x, text_obj.baseline); fontsize = text_obj.font_size; color = text_obj.color
            lines = new_text.split('\n'); line_height_factor = 1.2; line_height = fontsize * line_height_factor
            current_y = text_obj.baseline
            for i, line in enumerate(lines):
                line_point = fitz.Point(text_obj.x, current_y)
                try:
                    rc = page.insert_text(line_point, line, fontsize=fontsize, color=color, rotate=0, overlay=True, **font_arg)
                    if rc < 0: print(f"Warning: insert_text returned {rc} for line: {line}")
                except Exception as insert_e: print(f"ERROR: Insert failed: {insert_e}"); return False, f"Insert failed: {insert_e}"
                current_y += line_height

        # --- 3. Update State ---
        # ... (update text_obj properties as before) ...
        text_obj.text = new_text; text_obj.modified = True; text_obj.is_new = False
        print("DEBUG: EditableText object updated in memory.")

        print("DEBUG: apply_text_edit finished successfully.")
        return True, None

    # --- Error Handling ---
    except Exception as e:
        # ... (error handling as before) ...
        error_msg = f"Unexpected error in apply_text_edit: {e}"
...
def export_pdf_as_docx(doc, source_pdf_path, output_docx_path):
    """Exports PDF to DOCX using LibreOffice, with --infilter and enhanced debugging."""
    ...
    try:
        ...
        command = [
            ...
            '--invisible',
            '--nologo',
            '--infilter=writer_pdf_import', # CORRECTED: Use equals sign as per LO help
            '--convert-to', 'docx',
            '--outdir', str(temp_pdf_path_obj.parent), # e.g., /tmp
            str(temp_pdf_path)
        ]
        # The output DOCX will be named after the input temp_pdf_path stem,
        # and placed in temp_pdf_path_obj.parent (e.g., /tmp/)
        ...
        print(f"DEBUG [DOCX Export]: Running command: {' '.join(command)}") # This join is just for printing

        # For subprocess.run, each part of the command should be a separate item in the list
        # However, for options like --foo=bar, it's common to pass "--foo=bar" as a single item.

        current_env = os.environ.copy()
        process = subprocess.run(
            command, # command is already a list of strings
            capture_output=True,
            text=True,
            check=False,
            timeout=120,
        ...
        # --- Check for Output and Move/Rename ---
        if "0xc10" in process.stderr or "SfxBaseModel::impl_store" in process.stderr:
            ...
            # It's possible the "no export filter" error might also appear if the write error happens early.
        ...
        # Check for "no export filter" specifically if it's the primary error.
        ...
        # If return code was 0 and no specific errors caught above, proceed to find the file
        if os.path.exists(expected_output_location):
